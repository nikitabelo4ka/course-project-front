...
const Main = observer(() => {
  ...
  useEffect(() => {
    fetchAllItems().then((data) => {
      Object.entries(groupBy(data, 'collectionId'))
        .sort(function (a, b) {
      ...
      <div style={{ marginBottom: '5vw' }}>
        <div>
          ...
          <div className="d-flex align-items-center">
            ...
            <Button variant="warning" className="get-latest-items-btn" onClick={getLatestItems}>
              Get
            </Button>
          </div>
        </div>
        {latestItems === [] ? (
          <h1>Enter the desired number of items and click get</h1>
        ) : (
          <table className="table table-striped main-table">
            ...
            <tbody className="thead-dark main-table-body">
              {latestItems.map((item) => (
                <tr
                  key={item.id}
                  onClick={() => history(COLLECTION_ITEM_ROUTE + '/' + item.id)}
