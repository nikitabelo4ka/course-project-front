...
const Collection = observer(({collection}) => {
    ...
    return (
        <div className="collection-item">
            ...
            <div onClick={() => history(COLLECTION_ROUTE + '/' + collection.id)} className={token && (token.role === "ADMIN" || token.id === collection.userId) ? "collection-item-shadow" : "d-none"}></div>
            <p className="collection-item-name">{collection.name}</p>
            <div className={token && (token.role === "ADMIN" || token.id === collection.userId) ? "collection-item-shadow-buttons" : "d-none"}>
