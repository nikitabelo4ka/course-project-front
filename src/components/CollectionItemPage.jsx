    ...
    <Container>
      <div className="d-flex info-wrapper">
        <div className="d-flex">
          <div className="collection-item-page-column">
            {customFieldsKeys.map((item) => (
              <p key={item[1]} className="collection-page-info-item bold">
                {item[1]}:
              </p>
            ))}
          </div>
          <div className="collection-item-page-column values">
            {customFieldsValues.map((item, index) => (
              <p key={index} className="collection-page-info-item">
                {item === null
                  ? '-'
                  : typeof item === 'boolean'
                  ? item === true
                    ? 'Yes'
                    : 'No'
                  : new Date(item) !== 'Invalid Date' && !isNaN(new Date(item)) && typeof item !== 'number'
        ...
        <div>
          <div className="d-flex">
            {tags.map((tag) => (
              <p key={tag.id} className="collection-item-tag">
                #{tag[0].text}
              </p>
            ))}
          </div>
          <div className="d-flex align-items-center justify-content-center">
            <svg
              className={token ? (isItemLiked ? 'like' : 'dislike') : 'd-none'}
              onClick={() => token && checkLike(token.id, collectionItemInfo.id)}
        ...
        <div className="comments-wrapper">
            ...
            <div className="comment" key={comment.id}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                ...
                <div
                  ...
                  className={
                    token && (comment.userName === token.firstName || token.role === 'ADMIN')
          ...
          <Form className={token ? 'comment-send-wrapper' : 'd-none'} onSubmit={sendComment}>
            <input
              ...
              required
            />
            <Button variant="success" type="submit" className="comment-send-button">
              Send
            </Button>
          </Form>
        </div>
