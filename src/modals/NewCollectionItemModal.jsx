        ...
        if (isModify && collectionItemId !== 0) {
          fetchOneCollectionItem(collectionItemId).then((data) => {
            setCustomFieldsValue((state) => ({
              ...state,
              integer: { ...state.integer, integerField1: data.integerField1 },
            }));
            setCustomFieldsValue((state) => ({
              ...state,
              integer: { ...state.integer, integerField2: data.integerField2 },
            }));
            setCustomFieldsValue((state) => ({
              ...state,
              integer: { ...state.integer, integerField3: data.integerField3 },
            }));
            ...
            setCustomFieldsValue((state) => ({
              ...state,
              string: { ...state.string, stringField1: data.stringField1 },
            }));
            setCustomFieldsValue((state) => ({
              ...state,
              string: { ...state.string, stringField2: data.stringField2 },
            }));
            setCustomFieldsValue((state) => ({
              ...state,
              string: { ...state.string, stringField3: data.stringField3 },
            }));
            setCustomFieldsValue((state) => ({
              ...state,
              date: { ...state.date, dateField1: data.dateField1 === null ? '' : data.dateField1.slice(0, 10) },
            }));
            setCustomFieldsValue((state) => ({
              ...state,
              date: { ...state.date, dateField2: data.dateField2 === null ? '' : data.dateField1.slice(0, 10) },
            }));
            setCustomFieldsValue((state) => ({
              ...state,
              date: { ...state.date, dateField3: data.dateField3 === null ? '' : data.dateField1.slice(0, 10) },
            }));
      ...
      <Form onSubmit={addCollection}>
        <Modal.Body>
          ...
          <TagsInput
            ...
            onChange={setTags}
            name="tags"
            classNames={'tags-items'}
            placeHolder="Please press enter to add a tag"
          />
          {integerNames.map((item) => (
            <div key={item}>
              <Form.Label>{item[1]}</Form.Label>
              <Form.Control
                value={customFieldsValue.integer[item[0]]}
                onChange={(e) =>
                  setCustomFieldsValue((state) => ({
                    ...state,
                    integer: { ...state.integer, [item[0]]: e.target.value },
                  }))
                }
                type="number"
                className="collection-modal-input"
          ...
          {textNames.map((item) => (
            <div key={item}>
              <Form.Label>{item[1]}</Form.Label>
              <Form.Control
                value={customFieldsValue.text[item[0]]}
                onChange={(e) =>
                  setCustomFieldsValue((state) => ({ ...state, text: { ...state.text, [item[0]]: e.target.value } }))
                }
                type="text"
                className="collection-modal-input"
                required
          ...
          {stringNames.map((item) => (
            <div key={item}>
              <Form.Label>{item[1]}</Form.Label>
              <Form.Control
                value={customFieldsValue.string[item[0]]}
                onChange={(e) =>
                  setCustomFieldsValue((state) => ({
                    ...state,
                    string: { ...state.string, [item[0]]: e.target.value },
                  }))
                }
                as="textarea"
                className="collection-modal-input"
          ...
          {dateNames.map((item) => (
            <div key={item}>
              <Form.Label>{item[1]}</Form.Label>
              <Form.Control
                value={customFieldsValue.date[item[0]]}
                onChange={(e) =>
                  setCustomFieldsValue((state) => ({ ...state, date: { ...state.date, [item[0]]: e.target.value } }))
                }
                type="date"
                className="collection-modal-input"
                required
          ...
          {boolNames.map((item) => (
            <div key={item} className="d-flex">
              <Form.Label>{item[1]}</Form.Label>
              <Form.Check
                checked={customFieldsValue.bool[item[0]]}
                onChange={(e) =>
                  setCustomFieldsValue((state) => ({ ...state, bool: { ...state.bool, [item[0]]: e.target.checked } }))
                }
                type="switch"
                style={{ marginLeft: '0.6vw', marginTop: '0.1vw' }}
              />
            </div>
          ))}
        </Modal.Body>
